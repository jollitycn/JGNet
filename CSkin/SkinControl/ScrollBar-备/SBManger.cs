...
{
    internal class SBManger : NativeWindow, IDisposable
    {
        #region 变量
        ...
        internal SBManger(ScrollBar owner)
            : base()
        {
            _owner = owner;
            if ((Environment.OSVersion.Version.Major > 5) && NativeMethods.IsAppThemed()) {
                NativeMethods.SetWindowTheme(owner.Handle, "", "");
            }
            CreateHandle();
        }
        ...
        private class ScrollBarMaskControl : 
            MaskControlBase
        {
            private SBManger _owner;

            public ScrollBarMaskControl(SBManger owner)
