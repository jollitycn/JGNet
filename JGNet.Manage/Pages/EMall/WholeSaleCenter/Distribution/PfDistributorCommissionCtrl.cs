...
{
    public partial class PfDistributorCommissionCtrl : BaseUserControl
    {
        private DataGridViewPagingSumCtrl dataGridViewPagingSumCtrl;

        public PfDistributorCommissionCtrl()
        {
            InitializeComponent();
            ...
            dataGridViewPagingSumCtrl = new DataGridViewPagingSumCtrl(this.dataGridView1, dataGridViewPagingSumCtrl_CurrentPageIndexChanged, BaseButtonQuery_Click,
                new string[] { accruedCommissionDataGridViewTextBoxColumn.DataPropertyName });
        ...
        private void dataGridViewPagingSumCtrl_CurrentPageIndexChanged(int index)
        {
            if (this.para != null)
            {
                ...
                InteractResult<DistributorCommissionPage>   page = GlobalCache.ServerProxy.GetDistributors4Pf(para); ;
                //this.BindingCostumeStoreDataSource(page);
                if (page.ExeResult == ExeResult.Success)
                {
                    if (page.Data != null)
                    { 
                        dataGridViewPagingSumCtrl.BindingDataSource(page.Data?.Distributors, null, page.Data?.TotalEntityCount); 
                    }
                }
                else
                {
        ...
        private void BaseButtonQuery_Click(object sender, EventArgs e)
        {
            try
            {
                 ...
                 */
                  ...
                  para = new GetDistributorsPara()
                {
                    //StartTime=
                    DistributorIDOrName = this.skinTextBoxName.SkinTxt.Text,
                    PageIndex = 0,
                ...
                InteractResult<DistributorCommissionPage> result = GlobalCache.ServerProxy.GetDistributors4Pf(para);
                if (result.ExeResult == ExeResult.Success)
                {
                    if (result.Data != null)
                    {
                         this.dataGridViewPagingSumCtrl.Initialize(result.Data);
                        dataGridViewPagingSumCtrl.BindingDataSource(result.Data?.Distributors, null, result.Data?.TotalEntityCount);
