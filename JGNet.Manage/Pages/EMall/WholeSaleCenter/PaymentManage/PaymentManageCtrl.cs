...
{
    public partial class PaymentManageCtrl : BaseUserControl
    {
        private DataGridViewPagingSumCtrl dataGridViewPagingSumCtrl;

        public PaymentManageCtrl()
        {
            InitializeComponent();
            MenuPermission =RolePermissionMenuEnum.打款管理;
            dataGridViewPagingSumCtrl = new DataGridViewPagingSumCtrl(this.dataGridView1,
                ...
                new string[] { moneyDataGridViewTextBoxColumn.DataPropertyName}
                );
        ...
        //DistributorWithdrawRecord d;
          private void dataGridViewPagingSumCtrl_CurrentPageIndexChanged(int index)
           {
               if (this.para != null)
               {
                  // para.PageIndex = index;
                   InteractResult<DistributorWithdrawRecordPage>   page = GlobalCache.ServerProxy.GetDistributorPayManage(para); ;

                //GlobalCache.ServerProxy.GetDistributor("",Core.MyEnum.DistributorType.Pf)
                   //this.BindingCostumeStoreDataSource(page);
                   if (page.ExeResult == ExeResult.Success)
                   {
                       if (page.Data != null)
                       { 
                           dataGridViewPagingSumCtrl.BindingDataSource(page.Data.DistributorWithdrawRecords, null, page.Data?.TotalEntityCount); 
                       }
                   }
                   else
                   {
        ...
        GetDistributorWithdrawRecordPagePara para;
