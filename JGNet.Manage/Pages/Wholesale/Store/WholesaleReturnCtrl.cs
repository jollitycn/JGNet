...
{
    public partial class WholesaleReturnCtrl : BaseModifyUserControl
    {
      ...
      //  private DataGridViewPagingSumCtrl dataGridViewPagingSumCtrl3;
        ...
        private void Init()
        {
            MenuPermission = RolePermissionMenuEnum.批发退货;
            if (!HasPermission(RolePermissionMenuEnum.批发退货, RolePermissionEnum.单据时间))
            {
                dateTimePicker_Start.Enabled = false;
            }
        ...
        {
            int selectType = -1;
            WholesaleDeliveryReturnForm form = new WholesaleDeliveryReturnForm(pfCustomer,item.PfOrder.TotalPfPrice);
        ...
        private PfInfo Build()
        {
            ...
            string id = IDHelper.GetID(OrderPrefix.PfReturn, shop.AutoCode);
        ...
        /// <returns></returns>
        private CostumeStore BuildPfCostumeStore4(CostumeStore store ,List<PfCustomerStore> PfCostumeStore)
        {
            CostumeStore Item = null;
            if (PfCostumeStore != null && PfCostumeStore.Count>0)
            {
                List<PfCustomerStore> pfStore =PfCostumeStore.FindAll(t => t.CostumeID == store.CostumeID && t.ColorName == store.ColorName);
                if (pfStore.Count > 0)
                {
                    Item = new CostumeStore()
                    {
                        CostumeID = pfStore[0].CostumeID,
                        CostumeName = pfStore[0].CostumeName,
                        ColorName = pfStore[0].ColorName,
                        BrandID = pfStore[0].BrandID,
                        BrandName = pfStore[0].BrandName,
                        //  AllowReviseDiscount = pfStore[0].AllowReviseDiscount,
                        //  ShopID = pfStore[0].ShopID,
                        Price = pfStore[0].Price,
                        CostPrice = pfStore[0].CostPrice,
                        SumCount = pfStore[0].SumCount,
                        Remarks = pfStore[0].Remarks,
                        Title = "客户库存",
                        F = pfStore[0].F,
                        L = pfStore[0].L,
                        XS = pfStore[0].XS,
        ...
        {
            //添加到列表中
            //skinComboBox_PfCustomer.SelectedValue = costume.SupplierID;
            costumeFromSupplierTextBox1.Text = costume.ID;
            costumeFromSupplierTextBox1.Search();
            //  costumeFromSupplierTextBox1.Select();
            //   costumeFromSupplierTextBox1.Focus();
            //SendKeys.SendWait("{RIGHT}");
            //  SendKeys.Send(costume.ID);
        ...
        private void Save(bool isHang)
        {
            try
            {
                ...
                foreach (var detail in curInboundDetailList)
                {
                    ...
                    if (Math.Abs(detail.PfPrice) > 0 && Math.Abs(detail.PfPrice) > Convert.ToDecimal(99999999.99))
                    {
                        // SumPfMoney
                        pfPriceSuperLen = true;
                        break;
                    }
                    if (Math.Abs(detail.SumPfMoney) > 0 && Math.Abs(detail.SumPfMoney) > Convert.ToDecimal(99999999.99))
                    {
                        // SumPfMoney
                ...
                {
                    GlobalMessageBox.Show("退货单为空,不能退货！");
                    return;
                }
                ////弹窗录入付款方式
                ...
                if (isHang)
                {
                    result = GlobalCache.ServerProxy.HangUpPfReturn(item);
                }
                else
                {
                    result = GlobalCache.ServerProxy.PfReturn(item);
                }
                switch (result.ExeResult)
                {
                    case ExeResult.Error:
        ...
        private void baseButtonPick_Click(object sender, EventArgs e)
        {
            try
            {
                WholesaleDeliveryPickForm tiDanForm = new WholesaleDeliveryPickForm(false);
