...
{
    public partial class WholesaleBeginningStoreCtrl : BaseModifyUserControl
    {
        ...
        public WholesaleBeginningStoreCtrl()
        {
            InitializeComponent();
        ...
        private void DoSwitchShop()
        {
            try
            {
                ...
                List<PfCustomerStore> pfListPage = GlobalCache.ServerProxy.GetPfCostumeStores(PfCustomeID, costumeID, brand.AutoID);
                InitProgress(pfListPage.Count);
                this.curInboundDetailList = pfListPage;
        ...
        private void baseButtonSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (GlobalMessageBox.Show("确定保存该客户的期初库存吗？", "确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                {
                    return;
                }
                if (CommonGlobalUtil.EngineUnconnectioned(this)) { return; }
        ...
        {
            path = CJBasic.Helpers.FileHelper.GetPathToSave("保存文件", "客户期初库存录入" + new Date(DateTime.Now).ToDateInteger() + ".xls", Environment.GetFolderPath(Environment.SpecialFolder.Personal));
        ...
        private void DoExport()
        {
            try
            {
                ...
                {
                    List<PfCustomerStore> list = DataGridViewUtil.BindingListToList<PfCustomerStore>(dataGridView1.DataSource);
